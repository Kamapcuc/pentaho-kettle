...
public class ElasticSearchBulkDialog extends BaseStepDialog implements StepDialogInterface {
    ...
    private org.pentaho.di.ui.trans.steps.elasticsearchbulk.LabelTimeComposite wTimeOut;

    private Button wStopOnError;

    private Button wDocAsUpsert;

    private LabelComboVar wIdInField;

    private LabelComboVar wParentInField;
    ...
    private void fillIndexGroup(Composite parentTab) {
        ...
        FormData fdIndexGroup = new FormData();
    ...
    private void addServersTab() {
        CTabItem wServersTab = new CTabItem(wTabFolder, SWT.NONE);
        ...
        //Test button
        Button wTestCl = new Button(wServersComp, SWT.PUSH);
    ...
    private void updateOperationType() {
        wScript.setEnabled(ElasticSearchBulkMeta.OperationType.SCRIPT.equals(ElasticSearchBulkMeta.OperationType.valueOf(wOperationType.getText())));
        boolean isNotDelete = !ElasticSearchBulkMeta.OperationType.DELETE.equals(ElasticSearchBulkMeta.OperationType.valueOf(wOperationType.getText()));
        wFields.setEnabled(isNotDelete);
        wFields.setVisible(isNotDelete);
    }
    ...
    private void fillOptionsGroup(Composite parentTab) {
        ...
        Group wSettingsGroup = new Group(parentTab, SWT.SHADOW_NONE);
        ...
        // OperationType
        wOperationType = new LabelComboVar(transMeta, wSettingsGroup, SWT.SINGLE | SWT.LEFT | SWT.BORDER | SWT.DROP_DOWN, "Operation", "Type of operation");
        wOperationType.getComboWidget().setEditable(false);
        wOperationType.setItems(ElasticSearchBulkMeta.getOperationTypes());
        props.setLook(wOperationType);

        wOperationType.addSelectionListener(new SelectionAdapter() {
            @Override
        ...
        Label wlScript = new Label(wSettingsGroup, SWT.RIGHT);
        wlScript.setText("Script");
        props.setLook(wlScript);

        wScript = new TextVar(transMeta, wSettingsGroup, SWT.SINGLE | SWT.LEFT | SWT.BORDER);
        wScript.setEnabled(false);
        props.setLook(wScript);
        wScript.addModifyListener(lsMod);

        // Timeout
        wTimeOut = new org.pentaho.di.ui.trans.steps.elasticsearchbulk.LabelTimeComposite(wSettingsGroup,
        ...
        //Stop on error
        Label wlStopOnError = new Label(wSettingsGroup, SWT.RIGHT);
        ...
        //Stop on error
        Label wlDocAsUpsert = new Label(wSettingsGroup, SWT.RIGHT);
        wlDocAsUpsert.setText("Doc as upsert");

        wDocAsUpsert = new Button(wSettingsGroup, SWT.CHECK | SWT.RIGHT);
        ...
        wParentInField = new LabelComboVar(transMeta, wSettingsGroup, "Parent field", "Field to read parent from (optional)");
        props.setLook(wParentInField);
        wParentInField.getComboWidget().setEditable(true);
        wParentInField.addModifyListener(lsMod);
        wParentInField.addFocusListener(new FocusListener() {
            public void focusLost(org.eclipse.swt.events.FocusEvent e) {
            }

            public void focusGained(org.eclipse.swt.events.FocusEvent e) {
                getPreviousFields(wParentInField);
            }
        });
        getPreviousFields(wParentInField);

        Label wlIsOverwrite = new Label(wSettingsGroup, SWT.RIGHT);
        ...
        //Output rows
        Label wlUseOutput = new Label(wSettingsGroup, SWT.RIGHT);
        ...
        //use json
        Label wlIsJson = new Label(wSettingsGroup, SWT.RIGHT);
    ...
    public void getData(ElasticSearchBulkMeta in) {
        ...
        wOperationType.setText(in.getOperationType().name());
        wScript.setText(Const.NVL(in.getScript(), ""));
        ...
        wDocAsUpsert.setSelection(in.isDocAsUpsert());
        ...
        wParentInField.setText(Const.NVL(in.getParentInField(), ""));
        ...
        //Fields
        setFields(model.getFields(), wFields);

        //Servers
        for (ElasticSearchBulkMeta.SocketAddressHolder server : model.getServers())
            wServers.add(server.hostname, String.valueOf(server.port));

        wServers.removeEmptyRows();
        wServers.setRowNums();

        //Settings
        setSettings(model.getSettings(), wSettings);
    }

    private void setFields(Map<String, String> map, TableView table) {
        for (Map.Entry<String, String> entry : map.entrySet())
            table.add(entry.getValue(), entry.getKey());
        table.removeEmptyRows();
        table.setRowNums();
    }

    private void setSettings(Map<String, String> map, TableView table) {
        for (Map.Entry<String, String> entry : map.entrySet())
    ...
    private void toModel(ElasticSearchBulkMeta in) throws KettleException {// copy info to ElasticSearchBulkMeta
        ...
        in.setOperationType(wOperationType.getText());
        in.setScript(wScript.getText());
        ...
        in.setParentInField(wParentInField.getText());
        ...
        in.setDocAsUpsert(wDocAsUpsert.getSelection());
    ...
    private void test(TestType testType) {
        ...
        try {
            ...
            if (tempMeta.getServers().length > 0) {
                ...
                for (InetSocketTransportAddress address : tempMeta.getServersAddresses(variables)) {
